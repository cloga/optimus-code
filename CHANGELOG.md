...
- **Simplify `available-agents` defaults** — `init` / `upgrade` templates no longer emit redundant default ACP config for Claude Code and GitHub Copilot, and upgrade cleanup now removes legacy default ACP stanzas while preserving true custom transport overrides.
