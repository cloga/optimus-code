...
# core_principles
1. **The MCP Foundation**: Never couple core business logic to VS Code extensions. All heavy lifting, state management, parallel orchestration, and prompt assembly must happen in the `optimus-plugin` (the Node.js MCP Server). The UI (Copilot, Cursor) is just a "Thin Client".
2. **Stateless Headless Workers**: Agents dispatched by the Swarm must be headless CLI wrappers (`child_process.spawn`). They must not require human stdin. They must use strictly isolated SQLite/DB session IDs (`--session-id`) to prevent locks.
3. **Artifact-Driven Handoff**: Sub-agents do not communicate via memory-leaking conversational threads. They communicate by reading from and writing to Markdown files (e.g., `PROPOSAL.md`, `TODO.md`, `REVIEW.md`) inside the local `.optimus/` workspace.
4. **Resilience & Fallback**: An agent failing (Code 1) must never crash the server. Intercept `stderr` and convert failures into successful structured MCP JSON responses so the master orchestrator can adapt.
