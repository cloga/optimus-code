...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (request.params.name === "check_task_status") {
    ...
    if (task.status === 'running') {
      ...
      if (task.output_path) {
        ...
        } catch (e: any) { console.error('[check_task_status] Warning: failed to stat output_path:', e.message); }
      }
  ...
  if (request.params.name === "delegate_task_async") {
    ...
    if (!role || !task_description || !output_path || !workspace_path) {
        const _missing = [!role && "role", !task_description && "task_description", !output_path && "output_path", !workspace_path && "workspace_path"].filter(Boolean).join(", ");
        throw new McpError(ErrorCode.InvalidParams, `Invalid arguments for delegate_task_async: missing required parameter(s): ${_missing}`);
    }
  ...
  if (request.params.name === "dispatch_council_async") {
    ...
    if (!proposal_path || !Array.isArray(roles) || !workspace_path) {
        const _missing = [!proposal_path && "proposal_path", !Array.isArray(roles) && "roles (must be array)", !workspace_path && "workspace_path"].filter(Boolean).join(", ");
        throw new McpError(ErrorCode.InvalidParams, `Invalid arguments for dispatch_council_async: missing required parameter(s): ${_missing}`);
    }
  ...
  if (request.params.name === "dispatch_council") {
    ...
    if (fs.existsSync(configPath)) {
      try {
        ...
        } catch (e: any) { console.error('[roster_check] Warning: failed to read available-agents.json:', e.message); }
    }
      ...
      if (t2Files.length > 0) {
        for (const f of t2Files) {
          ...
          try {
            ...
            console.error('[roster_check] Warning: failed to read T2 role ' + roleName + ':', e.message);
            roster += `- ${roleName}\n`;
          }
        }
      } else {
    ...
    if (fs.existsSync(t3LogPath)) {
      ...
      } catch (e: any) { console.error('[roster_check] Warning: failed to load T3 usage stats:', e.message); }
    }
    ...
    if (fs.existsSync(skillsDir)) {
      const skillDirs = fs.readdirSync(skillsDir).filter(d => {
        try { return fs.statSync(path.join(skillsDir, d)).isDirectory() && fs.existsSync(path.join(skillsDir, d, 'SKILL.md')); } catch (e: any) { console.error('[roster_check] Warning: failed to stat skill dir:', e.message); return false; }
      });
  ...
  if (request.params.name === "remove_meta_cron") {
    ...
    try { if (fs.existsSync(lockPath)) fs.unlinkSync(lockPath); } catch (e: any) { console.error('[meta_cron_remove] Warning: failed to clean cron lock file:', e.message); }
