...
var SharedTaskStateManager = class _SharedTaskStateManager {
  ...
  static storageKey = "optimusTaskStates";
  static maxTasks = 25;
  ...
  buildPlannerPrompt(taskState, turnRecord, enrichedPrompt) {
    const previousTurns = taskState.turnHistory.filter((turn) => turn.turnId !== turnRecord.turnId);
    const recentTurns = previousTurns.filter((turn) => turn.executorOutcome).slice(-3).map((turn) => `Turn ${turn.sequence}:
  User: ${turn.prompt}
  Outcome: ${turn.executorOutcome.summary}`).join("\n");
    const taskSummary = taskState.latestSummary || "New task \u2014 no prior execution history.";
    const openQuestions = taskState.openQuestions.length ? taskState.openQuestions.slice(-5).map((q2) => `- ${q2}`).join("\n") : "- None recorded.";
    const blockedReasons = taskState.blockedReasons.length ? taskState.blockedReasons.slice(-5).map((r) => `- ${r}`).join("\n") : "- None recorded.";
    const rulesContent = this.readRulesMd();
    const rulesParts = rulesContent ? ["", "<project-rules>", rulesContent.trim(), "</project-rules>", ""] : [];
    const memoryContent = this.readMemoryMd();
    const memoryParts = memoryContent ? ["", "<project-memory>", memoryContent.trim(), "</project-memory>", ""] : [];
  ...
  buildExecutorPrompt(taskState, turnRecord, currentPrompt, synthesis, plannerIntent) {
    const previousTurns = taskState.turnHistory.filter((turn) => turn.turnId !== turnRecord.turnId);
...
Outcome: ${executorSummary}`;
    ...
    const openQuestions = taskState.openQuestions.length ? taskState.openQuestions.slice(-5).map((question) => `- ${question}`).join("\n") : "- None recorded.";
    const blockedReasons = taskState.blockedReasons.length ? taskState.blockedReasons.slice(-5).map((reason) => `- ${reason}`).join("\n") : "- None recorded.";
    const rulesContent = this.readRulesMd();
    const rulesParts = rulesContent ? ["", "<project-rules>", rulesContent.trim(), "</project-rules>", ""] : [];
    const memoryContent = this.readMemoryMd();
    const memoryParts = memoryContent ? ["", "<project-memory>", memoryContent.trim(), "</project-memory>", ""] : [];
  ...
  buildPlanSynthesisPrompt(taskState, turnRecord, currentPrompt, synthesis) {
    const rulesContent = this.readRulesMd();
    const rulesParts = rulesContent ? ["", "<project-rules>", rulesContent.trim(), "</project-rules>", ""] : [];
    const parts = [
   ...
   */
  buildDirectEscalatedPrompt(taskState, turnRecord, currentPrompt, enrichedPrompt, plannerInsight) {
    const previousTurns = taskState.turnHistory.filter((turn) => turn.turnId !== turnRecord.turnId);
    const recentTurns = previousTurns.slice(-3).map((turn) => {
      const executorSummary = turn.executorOutcome?.summary || "No executor outcome recorded.";
      return `Turn ${turn.sequence}: ${turn.prompt}
Outcome: ${executorSummary}`;
  ...
  buildDirectExecutorPrompt(taskState, turnRecord, currentPrompt, enrichedPrompt) {
    const previousTurns = taskState.turnHistory.filter((turn) => turn.turnId !== turnRecord.turnId);
    const recentTurns = previousTurns.slice(-3).map((turn) => {
      const executorSummary = turn.executorOutcome?.summary || "No executor outcome recorded.";
      return `Turn ${turn.sequence}: ${turn.prompt}
Outcome: ${executorSummary}`;
  ...
  async completeTurn(taskId, turnId, input) {
    const tasks = this.getTasks();
    ...
    if (input.executorOutcome?.status === "error") {
      taskState.blockedReasons = taskState.turnHistory.filter((turn) => turn.status === "failed" && turn.failureReason).map((turn) => turn.failureReason).slice(-5);
    } else {
  ...
  async failTurn(taskId, turnId, failureReason) {
    const tasks = this.getTasks();
    const taskState = tasks.find((task) => task.taskId === taskId);
    if (!taskState) {
      return void 0;
  ...
  getTask(taskId) {
    ...
    return taskState ? this.clone(taskState) : void 0;
  }
  ...
  async updateTaskSummary(taskId, summary) {
    const tasks = this.getTasks();
    const taskState = tasks.find((task) => task.taskId === taskId);
    if (!taskState) {
      return false;
  ...
  collectOpenQuestions(turnHistory) {
    return turnHistory.flatMap((turn) => turn.plannerContributions.flatMap((contribution) => contribution.openQuestions)).filter(Boolean).slice(-10);
  }
  buildReferencedTurnsContext(taskState, sequences) {
...
Executor outcome: ${outcome}
   ...
   */
  estimateContextTokens(taskState) {
    const parts = [
      taskState.title,
  ...
  getTasks() {
    return this.globalState.get(_SharedTaskStateManager.storageKey, []);
  }
  async saveTasks(tasks) {
    const limitedTasks = tasks.sort((left, right) => {
      if (left.pinned !== right.pinned) {
        return left.pinned ? -1 : 1;
      }
      return right.updatedAt - left.updatedAt;
    }).slice(0, _SharedTaskStateManager.maxTasks);
    await this.globalState.update(_SharedTaskStateManager.storageKey, limitedTasks);
  }
  buildId(prefix) {
