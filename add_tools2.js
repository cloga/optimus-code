...
const toolInsertStr = `
    ...
    const claudeToolOpus = vscode.lm.registerTool("optimus.claudeWorkerOpus", {
        ...
        async invoke(options, token) {
            ...
            if (!input.instruction) return new vscode.LanguageModelToolResult([new vscode.LanguageModelTextPart("Error: No instruction.")]);
    ...
    const copilotGeminiTool = vscode.lm.registerTool("optimus.copilotGeminiWorker", {
        ...
        async invoke(options, token) {
            ...
            try { return new vscode.LanguageModelToolResult([new vscode.LanguageModelTextPart(await adapter.invoke(input.instruction, "agent", () => {}) || "Done.")]); }
            catch (err) { return new vscode.LanguageModelToolResult([new vscode.LanguageModelTextPart("Error: " + String(err))]); }
        }
    });
